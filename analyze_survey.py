...
def area_1_enrollment_threat(df):
    ...
    # Note: Prompt mentioned Q33, but Q35 is the correct question ID for "Likelihood to enroll" in this dataset.
...
def area_2_roi_pressure(df):
    ...
    # Note: Prompt mentions "Required" or "Strongly Encouraged", but dataset values are "Yes"/"No".
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Analyze CPA Survey Data.')
    parser.add_argument('filepath', nargs='?', help='Path to the survey CSV file')
    args = parser.parse_args()

    if args.filepath:
        filepath = args.filepath
    else:
        # Find first CSV file that is not the summary file
        csv_files = [f for f in glob.glob("*.csv") if "cpa_pathway_summary.csv" not in f]
        if not csv_files:
            print("Error: No survey CSV file found.")
            exit(1)
        filepath = csv_files[0]
        print(f"Using survey file: {filepath}")
